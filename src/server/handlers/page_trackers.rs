...
/// Lists all page trackers for the authenticated user.
#[utoipa::path(
    tags = ["web_scraping"],
    responses(
        (status = 200, description = "List of page trackers.", body = [PageTracker]),
...
pub async fn page_trackers_list(
    ...
    let trackers = state.api.web_scraping(&user).get_page_trackers().await?;
