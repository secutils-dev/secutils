use crate::{
    users::UserId,
    utils::{PrivateKey, PrivateKeyAlgorithm, PrivateKeyEllipticCurve, PrivateKeySize},
};
...
impl TryFrom<(UserId, &PrivateKey)> for RawPrivateKey {
    type Error = anyhow::Error;

    fn try_from((user_id, item): (UserId, &PrivateKey)) -> Result<Self, Self::Error> {
        Ok(RawPrivateKey {
