    ...
    let (subject, text, html) = match content {
        Ok(content) => {
            ...
            (
                ...
                format!(
                    "\"{tracker_name}\" API tracker detected changes. Visit {back_link} to learn more.",
                ),
                api.templates.render(
                    "api_tracker_changes_email",
                    &json!({
        ...
        Err(error_message) => (
            ...
            format!(
                "\"{tracker_name}\" API tracker failed to check for changes due to the following error: {error_message}. Visit {back_link} to learn more."
            ),
            api.templates.render(
                "api_tracker_changes_error_email",
                &json!({
                    "tracker_name": tracker_name,
