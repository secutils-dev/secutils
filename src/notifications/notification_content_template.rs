...
pub use self::templates::{AccountActivationTemplate, PasswordResetTemplate};
...
impl NotificationContentTemplate {
    ...
    pub async fn compile_to_email<DR: DnsResolver, ET: EmailTransport>(
        ...
        match self {
            ...
            NotificationContentTemplate::PasswordReset(template) => {
                template.compile_to_email(api).await
            }
        }
    }
...
mod tests {
    use crate::{
        notifications::{AccountActivationTemplate, NotificationContentTemplate},
