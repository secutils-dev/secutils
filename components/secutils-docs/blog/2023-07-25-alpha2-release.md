...
description: "Secutils.dev 1.0.0-alpha.2 ships the first web page resources tracker (now Page tracker), plus bug fixes, schema work, and CI/CD automation."
slug: alpha2-release
authors: azasypkin
image: https://secutils.dev/docs/img/blog/goal.png
tags: [announcement, release, thoughts]
keywords: [secutils.dev, 1.0.0-alpha.2, page tracker, web page resources tracker, release notes, indie hacker, database schema migration]
---
...
The "Resources tracker" / "Page tracker" feature has evolved significantly since this post. Today both the original Resources Tracker and the later Content Tracker are unified under a single [**Page tracker**](https://secutils.dev/docs/guides/web_scraping/page) feature, scheduling and execution are handled by the standalone [**Retrack**](https://github.com/secutils-dev/retrack) project, and there is also a separate [**API tracker**](https://secutils.dev/docs/guides/web_scraping/api) for tracking arbitrary HTTP API responses. The video embed below still works, but the UI shown is from the alpha era.
:::
