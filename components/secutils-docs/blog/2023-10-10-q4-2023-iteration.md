...
description: "Plan for the Secutils.dev Q4 2023 iteration: arbitrary web content tracking, per-user webhook subdomains, inherited Content Security Policies, and shareable certificate templates."
slug: q4-2023-iteration
authors: azasypkin
image: /img/blog/2023-10-10_subdomain_responders.png
tags: [thoughts, overview, technology]
keywords: [secutils.dev, q4 2023 roadmap, content tracker, webhook subdomain, csp inheritance, certificate template sharing, mocking webhooks]
---
Hello!
...
All four workstreams below shipped, and the surface area has grown further:

- **Arbitrary web content tracking** is now part of the unified [**Page tracker**](https://secutils.dev/docs/guides/web_scraping/page) feature, alongside resource tracking and [**API trackers**](https://secutils.dev/docs/guides/web_scraping/api). Custom JavaScript extractors can also be imported directly from Playwright codegen output.
- **Per-user webhook subdomains** shipped, but the UI label has changed: each responder now configures a [**subdomain prefix**](https://secutils.dev/docs/guides/webhooks) (e.g. `myapp.webhooks.secutils.dev`) instead of "custom subdomains". MITM responders were added later to intercept and modify proxied requests.
- **Inherited CSP** policies can now be created from a raw policy string or fetched from any URL.
- **Shareable certificate templates** are available, including the ability to generate a fresh key/certificate pair from a public template link.
:::
