...
const EXTRACTOR_IMPORT_ACTIONS: ImportAction[] = [
  {
    id: 'playwright-recording',
    label: 'Import: Playwright recording',
    description: (
      <>
        <p>
          Paste a script generated by Playwright codegen (<code>npx playwright codegen</code>), the Playwright VS Code
          extension, or any Playwright recorder browser extension. Both test-framework and library formats are
          supported.
        </p>
        <EuiSpacer size="xs" />
        <p>
          <EuiLink href="/docs/guides/web_scraping/page#record-a-content-extractor-script" target="_blank" external>
            Learn more about recording content extractor scripts
          </EuiLink>
        </p>
      </>
    ),
    transform: transformPlaywrightScript,
  },
  {
    id: 'devtools-recording',
    label: 'Import: Chrome DevTools recording',
    description: (
      <>
        <p>
          Paste a JSON recording exported from the Chrome DevTools Recorder panel (DevTools → Recorder → Export as
          JSON).
        </p>
        <EuiSpacer size="xs" />
        <p>
    ...
    transform: transformDevToolsRecording,
  },
];
...
export default function PageTrackerEditFlyout({ onClose, tracker }: Props) {
  ...
  const onSave = useCallback(() => {
    ...
    const [requestPromise, successMessage, errorMessage] = tracker?.id
      ? [
          apiFetch(`/api/web_scraping/page_trackers/${tracker.id}`, { method: 'PUT', body: trackerRequestBody }),
          `Successfully updated "${name}" page tracker`,
          `Unable to update "${name}" page tracker, please try again later`,
        ]
      : [
          apiFetch('/api/web_scraping/page_trackers', { method: 'POST', body: trackerRequestBody }),
          ...
          <EuiFormRow
            ...
            helpText={
              <span>
                ...
                Right-click the editor to import a recorded Playwright script or Chrome DevTools recording.
              </span>
            }
          >
            <ScriptEditor
